...
export const Result: React.FC = () => {
    ...
    // Calculate result
    // We need to check if the order was correct.
    // The backend sends `resultInvalidCardIds`.
    const invalidCardIds = roomState.resultInvalidCardIds || [];
    const isSuccess = invalidCardIds.length === 0 && !isEnded; // If ended, it might be game over or clear

    // If ENDED, check message
    const isGameOver = isEnded && roomState.currentLifes <= 0;
    const isGameClear = isEnded && !isGameOver; // Simplified

    return (
        <div className="min-h-screen bg-gray-100 p-6 flex flex-col items-center">
            <h1 className="text-4xl font-bold mb-8 text-gray-800">
                {isEnded ? (isGameOver ? "GAME OVER" : "GAME CLEAR!") : (isSuccess ? "ROUND CLEAR!" : "FAILED...")}
            </h1>
            ...
            <div className="bg-white p-6 rounded-lg shadow-md w-full max-w-2xl text-center">
                <div className="flex justify-center gap-8 mb-6">
                    <div className="text-center">
                        <div className="text-sm text-gray-500">Lifes</div>
                ...
                {isVotingPhase && (
                    <div className="mt-6 border-t pt-6">
                        <h3 className="text-xl font-bold mb-4">Vote: Continue with same hand?</h3>
                        ...
                        {myPlayer?.vote ? (
                            <div className="text-lg font-medium text-gray-600">
                                You voted: <span className="font-bold">{myPlayer.vote}</span>
                            </div>
                        ) : (
                        ...
                        <div className="mt-4 text-sm text-gray-400">
                            Votes: {roomState.players.filter(p => p.vote).length} / {roomState.players.filter(p => p.role === 'PLAYER').length}
                        </div>
                    </div>
                )}
